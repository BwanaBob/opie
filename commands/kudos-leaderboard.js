  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("episodes")
        .setDescription(
          "Comma-separated list of episodes, or a number for N most recent episodes (e.g. 5 or S01E01,S01E02)"
        )
        .setRequired(true)
    )
    .addChannelOption((option) =>
      option
        .setName("channel")
        .setDescription(
          "Channel to display leaderboard in (defaults to this channel)"
        )
        .setRequired(false)
    ),
  async execute(interaction) {
    ...
    if (!episodes.length) {
      await interaction.reply({
        content:
          "You must provide at least one episode name or a valid number.",
