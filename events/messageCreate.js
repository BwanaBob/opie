	...
	async execute(message) {
        if(( message.attachments.size !== 0  || message.embeds.length !== 0 ) && !message.member.roles.cache.has('343568731793915904')){
            ...
            if(lastTime == undefined){
                console.log(`[${uniDate}] EMB | ${message.guild.name} | ${message.channel.name} | ${message.member.displayName} (${message.author.tag}) | First Embed`)
                message.client.timers.set(message.member.id, message.createdTimestamp)
            } else {
                const elapsed = Math.trunc((message.createdTimestamp - lastTime) /1000)
                if(elapsed < gifDelay) {
                    console.log(`[${uniDate}] EMB | ${message.guild.name} | ${message.channel.name} | ${message.member.displayName} (${message.author.tag}) | ${elapsed}sec BAD`)
                    message.delete()
                    ...
                    console.log(`[${uniDate}] EMB | ${message.guild.name} | ${message.channel.name} | ${message.member.displayName} (${message.author.tag}) | ${elapsed} = OK`)
                    message.client.timers.set(message.member.id, message.createdTimestamp)
                }
            }
        }
